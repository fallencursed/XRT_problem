...
bool QueryDatabase1()
{
	strcpy_s(query, "select * from user"); //执行查询语句，这里是查询所有，user是表名，不用加引号，用strcpy也可以  
...
{
	//可以想办法实现手动在控制台手动输入指令  
	strcpy_s(query, "insert into user values (NULL, 'Lilei', 'wyt2588zs','lilei23@sina.cn');");
...
bool ModifyData()
{
	strcpy_s(query, "update user set email='lilei325@163.com' where name='Lilei'");
	if (mysql_query(&mysql, query))        //执行SQL语句  
	{
		printf("Query failed (%s)\n", mysql_error(&mysql));
		return false;
